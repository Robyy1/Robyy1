...
function sanitizeGeneralText(text) {
  ...
  try {
    ...
    if (!mode || !["general", "code", "dictionary"].includes(mode)) {
      return res
        ...
        .json({
          error:
            'Mode is required and must be "general", "code", or "dictionary"',
        });
    }
    ...
    if (mode === "general") {
      ...
      const cleanedText = sanitizeGeneralText(quote.text);

      return res.json({
        mode: "general",
        text: cleanedText || quote.text,
        length: quote.length,
      });
    }
    ...
    if (mode === "code") {
      ...
      if (!snippet) {
        return res
          .status(404)
          .json({ error: "No code snippet available for the given criteria" });
      }
...
router.get("/list", (req, res) => {
  try {
    ...
    if (!mode || !["general", "code"].includes(mode)) {
      return res
        .status(400)
        .json({ error: 'Mode is required and must be "general" or "code"' });
    }
...
router.get("/languages", (_req, res) => {
  const languages = [
    ...new Set(codeSnippetsData.map((s) => s.language)),
  ].sort();
  res.json({ languages });
});
